...
本项目由 211 高校研究生开展，定位是偏学术研究、偏变化检测方法创新，而不是生产级工程项目。工作的首要评价标准是：方法动机是否合理、创新点是否清晰、对照和消融是否严谨、四个数据集尤其 SYSU-CD 上的效果是否真实有效，以及能否形成可信的论文证据链。代码工程化服务于实验，不应反客为主；不要把主要精力投入与研究结论无直接关系的企业级架构、过度抽象、通用平台化、部署基础设施或形式化重构。

这不等于忽略代码质量。凡是会影响算法语义、指标可信度、公平对比、复现性、显存可运行性、断点续训和测试集隔离的工程问题仍必须修正；其他纯工程优化应保持最小、清晰，并说明它对方法验证的实际价值。
...
提出后续工作时，优先级应是“方法假设与创新性 → 低成本诊断实验 → 严格消融与跨数据集效果 → 必要的代码实现 → 可选工程优化”。不要仅因代码不够通用或不符合生产项目习惯，就建议大规模重写已经能稳定支持实验的训练框架。
